...
- [/] 行単位マージを geodiff で（設計は [[docs/technical/drive-geodiff-sync]]。ops.log 案は取り下げ）
  ...
  - [x] 段5（本物の Drive）: Fold（自動同期）と PC の Drive for Desktop で 2 回往復。別々の行はそろい、同じ行は衝突 1 件で端末の値が残った（2026-09-24）
  - [x] 実機で「クラウドの値に戻す」を押す（PC 側まで戻った）／直したあと自動同期が静かになるのを見る（2026-09-24）
  - [x] 🐛 フォルダの push が 1 つ変わると全ファイルを上げ直していた → 変わったものだけ（2026-09-24）
  - [x] 🐛 Drive 連携フォルダの読み込みが `.sync` を除いておらず、base がレイヤとして見えていた（2026-09-24）
  - [x] 🐛 同期のたびに `.qgs` と `.kmeta.json` を同じ中身で書き直し、自動同期が 5 分ごとにアップロードしていた（2026-09-24）
  - [x] 🐛 web の「フォルダを選択」は読み込み直後の 1 回目が失敗する → プロジェクトを開くまでは Drive の復元をクリックに便乗させない（2026-09-24）
  - [x] 🐛 Drive クローンのダイアログがキーボード表示時に下端 15px はみ出す（2026-09-24）
  - [x] 🐛 属性フォームが Enter でしか保存せず、欄を離れると入力が捨てられる／数値の列で日本語キーボードが出る（2026-09-24）
  - [x] 🐛 キーボード表示中に地図の左の道具列がはみ出す（2026-09-24）
