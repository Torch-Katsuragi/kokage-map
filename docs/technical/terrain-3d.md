...
### 読み込みの最適化（2026-09-13 夕、松本「テクスチャ・背景地図まわりを一度本格的に」）

実測（Pixel 9・freespot の Wi-Fi ≈ 100 KB/s、淡色 + 等高線）: HTTP 656 回 / 25 分、1 回 3〜9 秒、タイル 1 枚 17〜41 秒。
PC からは同じタイルが 0.1〜0.15 秒で取れるのでサーバは速い。**回線の帯域が支配的**で、効くのは「バイト数と回数を減らす」こと。
...
- **一括ダウンロードは見えているレイヤ全部**（`BaseMapService.downloadableProviders`。OSM は除外して通知、等高線は作ってキャッシュへ）。
  段の範囲はプロバイダの和で 18 まで。推定枚数は 枚数 × プロバイダ数

数字の取り方: `[TILE] … http Nms` の分布、`[3D] tile … load Nms (dem M)`、`/proc/net/dev` の wlan0 差分（10 秒で 1 MB = 100 KB/s）
