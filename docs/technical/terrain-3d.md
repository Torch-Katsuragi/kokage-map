...
| 引いた段の間引き | セル × step ≥ 30m では面の輪郭・面と点のラベルを省き、格子の切り分けを粗く（最低 4 セル）、点は 60px 相当の格子でまとめて数を出す |
...
| メッシュ生成 | フレーム合計 20ms まで。超えたぶんは手持ちの段か穴埋めで繋いで次のフレームに |
...
- **標高ソースは連なり**（`DemTileSource.defaultCascade`、2026-09-09）: 地理院 DEM1A（航空レーザ 1m、z17 まで）→ DEM5A（5m、z15）→
  DEM10B（10m、z14）→ AWS Terrain Tiles（30m 級、全球）。**点ごとに重ねる**: 細かいソースの無効な点（整備範囲外・水面。
